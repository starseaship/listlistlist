import './styles-vocab-sidebar.css';
...
function renderHome() {
  const recommendedQuestions = state.questions.slice(0, 3);
  ...
  return appShell(`
    ...
    <section class="panel home-study-panel">
      <div class="section-head">
        <div>
          <h2>今天复习什么？</h2>
          <p>按 JLPT / TOEIC、题型、状态筛选错题；用闪卡复习，每次随机打乱当前题目的选项。</p>
        </div>
      </div>

      <div class="home-study-actions">
        <button class="btn secondary" data-home-action="unmastered-review">从未掌握开始复习</button>
        <button class="btn secondary" data-home-action="jlpt-grammar">先刷 JLPT 文法</button>
        <button class="btn secondary" data-home-action="toeic-part5">再看 TOEIC Part 5</button>
      </div>
      ...
      <div class="home-word-search">
        ...
        <div class="home-search-row">
          ...
          <button class="btn" id="homeWordSearchBtn">搜索</button>
        </div>
      </div>
    </section>
    ...
    <section class="panel recommended-panel">
      <div class="section-head">
        ...
        <button class="btn secondary" data-go="review">开始复习</button>
      </div>
      <div class="home-recommended-list">
        ${recommendedQuestions.map(homeRecommendedCard).join('')}
      </div>
    </section>
  `);
}

function homeRecommendedCard(q) {
  const suggestion = q.status === 'mastered' ? '暂不需要' : q.status === 'uncertain' ? '今天再看' : '现在复习';
  const levelOrPart = q.section || q.level || '';

  return `
    <article class="home-recommended-card" data-detail="${q.id}">
      <div class="meta-tags">
        <span class="tag">${escapeHtml(q.exam_category)}</span>
        <span class="tag lavender">${escapeHtml(levelOrPart)}</span>
        ${statusTag(q.status)}
      </div>
      <h3>${escapeHtml(q.question_text)}</h3>
      <div class="subline">上次复习：${escapeHtml(q.last_reviewed_at || '未复习')} · 建议：${suggestion}</div>
    </article>
  `;
}
...
function renderVocab() {
  ...
  const sidebarContent = sidebarCollapsed ? `
    <div class="vocab-collapsed-rail">
      <button class="btn secondary vocab-collapse-btn" id="toggleVocabSidebar" aria-label="展开生词列表">展开</button>
      <div class="vocab-rail-title">生词列表</div>
      <span class="tag">${list.length} 个</span>
    </div>
  ` : `
    <div class="results-topbar">
      <div class="section-head" style="margin:0"><div><h2>生词列表</h2><p>${list.length} 个结果</p></div></div>
      <button class="btn soft vocab-collapse-btn" id="toggleVocabSidebar" aria-label="收起生词列表">← 收起</button>
    </div>
  ...
  return appShell(`
    ...
    <section class="two-col vocab-layout ${sidebarCollapsed ? 'collapsed' : ''}">
...
function attachEvents() {
  ...
  document.querySelectorAll('[data-home-action]').forEach(el => el.addEventListener('click', () => handleHomeAction(el.dataset.homeAction)));
  const homeWordSearch = document.getElementById('homeWordSearch');
  const homeWordSearchBtn = document.getElementById('homeWordSearchBtn');
  const runHomeWordSearch = () => {
    const keyword = (homeWordSearch?.value || '').trim();
    if (!keyword) return;
    state.search = keyword;
    go('questions');
  };
  if (homeWordSearchBtn) homeWordSearchBtn.addEventListener('click', runHomeWordSearch);
  if (homeWordSearch) homeWordSearch.addEventListener('keydown', event => { if (event.key === 'Enter') runHomeWordSearch(); });
  const toggleVocabSidebar = document.getElementById('toggleVocabSidebar');
  if (toggleVocabSidebar) toggleVocabSidebar.addEventListener('click', () => { state.vocabListCollapsed = !state.vocabListCollapsed; render(); });
...
function handleHomeAction(action) {
  if (action === 'unmastered-review') {
    const target = state.questions.find(q => q.status === 'unmastered') || state.questions[0];
